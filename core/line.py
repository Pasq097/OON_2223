...
class Line:
    ...
    def ase_generation(self, number_of_amplifiers):
        # ASE = N (h * f * Bn * NF * [G-1]             # spontaneus emission generated by a cascade of amp.
                                                       # in linear units
        f = 193.414 * 10**12                           # THz C-band center
        B_n = 12.5 * 10**9                             # GHz noise bandwidth
        h = 6.62607015 * 10**-34
    ...
    def nli_generation(self, R_s, delta_f, P_ch, n_span):
        ...
        b = 2 * alpha_lin
        ni_nli = (16 / (27*pi)) * (math.log10(a/b) * (alpha_lin/beta) * ((self._gamma ** 2) * (L_eff ** 2) / (R_s ** 3))) # Nonlinear interference
        NLI = P_ch**3 * ni_nli * n_span * B_n    # P_NLI is the variance(power) of the NLI disturbance on the received signal constellation
    ...
    def noise_generation(self, number_of_amplifiers,  R_s, delta_f, P_ch):
        ...
        ni_nli = self.nli_generation(R_s, delta_f, P_ch, number_of_amplifiers)[1]       # I use this only to pass the variable nli
        noise_power = noise_contribution_1 + noise_contribution_2                       # The total noise is the sum of the two contribution
    ...
    def propagate(self, light_path):
        ...
        for node in self._successive:
            self._successive[node].propagate(light_path)                   # propagate the signal on the node
